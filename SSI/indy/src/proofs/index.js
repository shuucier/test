...
exports.getProofRequests = async function(force) {
    if(force || !proofRequests) {
        ...
        let transcriptCredDef = await indy.issuer.getCredDefByTag('Validated Credential');
        let transcriptCredDef1 = await indy.issuer.getCredDefByTag('Career Credential');
        let transcriptCredDef2 = await indy.issuer.getCredDefByTag('Verified Insurance');
        if(transcriptCredDef) {
            proofRequests['Verified Information'] = {
        ...
        else if(transcriptCredDef1) {
            proofRequests['Verified Career Info'] = {
                name: 'Verified Career Info',
                version: '0.1',
                requested_attributes: {
                    'attr5_referent': {
                        'name': 'degree',
                        'restrictions': [{'cred_def_id': transcriptCredDef1.id}]
                    },
                    'attr6_referent': {
                        'name': 'workingyear',
                        'restrictions': [{'cred_def_id': transcriptCredDef1.id}]
        ...
        else if(transcriptCredDef2) {
            proofRequests['Verified Insurance Info'] = {
                name: 'Verified Insurance Info',
                version: '0.1',
                requested_attributes: {
                    'attr10_referent': {
                        'name': 'medicalId',
                        'restrictions': [{'cred_def_id': transcriptCredDef2.id}]
                    },
                    'attr11_referent': {
                        'name': 'policyNumber',
                        'restrictions': [{'cred_def_id': transcriptCredDef2.id}]
